...
class HuggingFaceDeployer:
    ...
    def prepare_for_upload(self, output_dir: str = "./hf_model"):
        ...
        # Load checkpoint - handle both checkpoint.pt and best_model.pt
        ...
        if config_path.exists():
            ...
            # Create default config for best_model.pt
            config = {
                'num_bins': 4096,
        ...
        model = ImprovedDistanceAwareChronos(
            ...
            num_bins=config.get('num_bins', 4096),
            device='cpu'
        )
        
        # Load state dict - only the distance_output layer was trained
        distance_output_state = checkpoint.get('model_state_dict', checkpoint)
        model.distance_output.load_state_dict(distance_output_state)
        
        # Save model components
        print("Saving model components...")
        
        # Save the base Chronos/T5 model
        model.base_model.save_pretrained(output_path / "base_model")
        
        # Save distance-aware components
        torch.save(
            distance_output_state,
        ...
        model_config = {
            ...
            'num_bins': config.get('num_bins', 4096),
            'training_epoch': config.get('epoch', 'N/A'),
            'val_loss': config.get('val_loss', 0.0),
